...
st.title("SafeSpeak AI - Ethics Assistant")
st.caption(
...
with tab_text:
    ...
    st.caption("Analyse a text comment or post")
    ...
    if analyze_btn:
        if not user_text.strip():
            ...
            with st.spinner("Analysing ethics & risk..."):
                try:
...
with tab_image:
    ...
    st.caption("Upload a screenshot of a comment section")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Analyse image"):
            try:
                ...
                st.error(f"Error while analysing image with LLM: {e}")
...
with tab_audio:
    ...
    st.caption("Speak and get a live risk analysis")
        ...
        if st.session_state.last_audio_size != current_size:
            with st.spinner("Analysing ethics & risk..."):
                try:
                    ...
                    st.error(f"Error while analysing audio with LLM: {e}")
