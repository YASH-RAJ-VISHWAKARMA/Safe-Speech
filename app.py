...
# -----------------------------
st.set_page_config(
    page_title="EthicShield – AI Social Media Ethics Assistant",
    page_icon="🛡️",
    layout="wide",
)

st.title("🛡️ EthicShield – AI Ethics Assistant for Social Media")
st.caption(
...
tab_text, tab_image, tab_audio = st.tabs(
    ["✍️ Text input", "🖼️ Screenshot / image", "🎙️ Voice / audio"]
)
...
with tab_text:
    ...
    if analyze_btn:
        if not user_text.strip():
            ...
            with st.spinner("Talking to Gemini and analysing ethics & risk..."):
                try:
    ...
    if uploaded_file is not None:
        ...
        if st.button("Analyse image"):
            try:
                ...
                with st.spinner("Reading text from image and analysing with Gemini..."):
                    result = call_gemini_for_image(image_bytes, mime_type)
                    render_risk_box(result)
            except Exception as e:
                st.error(f"Error while analysing image with Gemini API: {e}")

# -----------------------------
# Audio / Voice tab
# -----------------------------
# -----------------------------
# Live mic / Voice tab (near real-time)
# -----------------------------
with tab_audio:
    st.subheader("🎙️ Speak and get a live risk analysis")
    ...
    # This widget records from the browser microphone and returns an UploadedFile (WAV)
    audio_value = st.audio_input(
        "Press the mic button, speak, then stop recording",
        sample_rate=16000,  # good for speech analysis
    )
    ...
    if audio_value:
        ...
        # To avoid re-calling the API on every rerun with the same audio,
        # we track the last processed file size in session_state.
        if "last_audio_size" not in st.session_state:
            st.session_state.last_audio_size = None

        current_size = len(audio_value.getvalue())

        # Only analyse if this is a new recording
        if st.session_state.last_audio_size != current_size:
            with st.spinner("Listening to your voice and analysing with Gemini..."):
                try:
                    ...
                    st.error(f"Error while analysing audio with Gemini API: {e}")

        # Show the latest result (if available)
...
#     st.write(
#         "This tool is meant to **support responsible and empathetic communication**.\n\n"
#         "- It does *not* replace human judgement.\n"
#         "- It may sometimes over- or under-estimate risk.\n"
#         "- Always think about the impact of your words, voice tone, and images on real people."
#     )
#     st.markdown("---")
